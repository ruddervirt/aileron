...
type BuildVMOverride struct {
	...
	// For an override NIC whose name matches a base-spec NIC, the identity
	// fields mac, slot, and model are taken from the base NIC (when set
	// there) and any conflicting override values are ignored: guest OS
	// state binds to NIC identity (e.g., Windows keys DHCP reservations
	// and leases to the MAC), and clones boot with the base values, so a
	// build that ran with different identity would bake guest state no
	// clone could ever match.
	// +optional
	NICs []VMNIC `json:"nics,omitempty"`
}
